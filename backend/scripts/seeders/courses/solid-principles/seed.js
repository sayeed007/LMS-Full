...
async function seedSOLIDPrinciplesCourse(users, categories) {
  console.log('\\n📚 Seeding SOLID Principles course...');
  
  try {
    const instructor = users.find(u => u.email === 'jane.instructor@lms.com');
    const category = categories.find(c => c.name === 'Software Engineering');
    ...
    console.log(`\\n✅ SOLID Principles course seeded successfully!`);
    return course;
  } catch (error) {
    console.error('❌ Error seeding SOLID Principles course:', error.message);
    throw error;
  }
}
