...
const getStatusBadgeClass = (status: string): string => {
  ...
  if (statusLower === "yet to start" || statusLower === "not started")
...
export default function IndividualCourseReportPage() {
  ...
  const {
    ...
    error: coursesError,
  } = useGetCoursesListQuery(searchTerm);
  ...
  const {
    ...
    error: reportError,
  } = useGetIndividualCourseReportQuery(selectedCourse || "", {
    skip: !selectedCourse,
  });
  ...
  // PDF export
  const [exportPDF, { isLoading: isExportingPDF }] =
    useLazyExportIndividualCoursePDFQuery();
      ...
      {!selectedCourse && courses.length > 0 && (
        <div className="flex flex-col w-full h-[60vh] justify-center items-center">
          ...
          <p className="text-gray-500 text-sm">
            Use the dropdown above to choose a course
          </p>
        </div>
      )}
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6 mb-8">
              ...
              <EngagementMetricsChart
                data={lessonStats.slice(0, 10).map((lesson) => ({
                  name:
                    lesson.lesson.length > 20
                      ? lesson.lesson.substring(0, 20) + "..."
                      : lesson.lesson,
                  enrollments:
                    lesson.yetToStart + lesson.inProgress + lesson.completed,
